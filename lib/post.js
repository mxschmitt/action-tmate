...
function getTmateExecutablePath() {
  if (process.platform === "win32") {
    ...
    lib_core.getInput("install-dependencies") !== "false" &&
    process.platform !== "darwin"
  ) {
    return external_path_default().join(external_path_default().join(external_os_default().tmpdir(), "tmate"), "tmate");
  } else {
    return "tmate";
  }
}
...
async function waitUntilDebuggingSessionExit() {
  ...
  while (true) {
    ...
    await sleep(parseInt(lib_core.getInput("wait-interval")));

    if (
      lib_core.getInput("check-num-clients") !== "false" &&
      !(await doesTmateHaveConnectedClients())
    ) {
      lib_core.info("Exiting debugging session because 'tmate' has no clients");
      break;
    }
  }
}
...
async function doesTmateHaveConnectedClients() {
  const tmate = getTmate();
  const tmateNumClients = await execShellCommand(
...
function didTmateQuit() {
  return !external_fs_default().existsSync(getTmateSocketPath());
}
...
if (lib_core.getInput("wait-in-post") !== "false") {
